...
class AnalyticsData(ExtraBaseModel):
    ...
    ueMobilityInfos: Optional[List[UeMobilityExposure]] = Field(
        None, description="", min_items=1
    )
    ueCommInfos: Optional[List[UeCommunication]] = Field(
        None, description="", min_items=1
    )
    nwPerfInfos: Optional[List[NetworkPerfExposure]] = Field(
        None, description="", min_items=1
    )
    abnormalInfos: Optional[List[AbnormalExposure]] = Field(
        None, description="", min_items=1
    )
    congestInfos: Optional[List[CongestInfo]] = Field(None, description="", min_items=1)
    qosSustainInfos: Optional[List[QosSustainabilityExposure]] = Field(
        None, description="", min_items=1
    )
    disperInfos: Optional[List[DispersionInfo]] = Field(
        None, description="", min_items=1
    )
    dnPerfInfos: Optional[List[DnPerfInfo]] = Field(None, description="", min_items=1)
    svcExps: Optional[List[ServiceExperienceInfo]] = Field(
        None, description="", min_items=1
    )
    disperReqs: Optional[List[DispersionRequirement]] = Field(
        None, description="", min_items=1
    )
...
class TargetUeId(ExtraBaseModel):
    ...
    gpsi: Optional[constr(regex=r"^(msisdn-[0-9]{5,15}|extid-.+@.+|.+)$")] = Field(
        None, description=""
    )

    exterGroupId: Optional[str] = Field("Group1@domain.com", description="")
...
class AnalyticsRequest(ExtraBaseModel):
    ...
    analyEventFilter: Optional[AnalyticsEventFilter] = None
    analyRep: Optional[EventReportingRequirement] = None
    tgtUe: Optional[TargetUeId] = None
    suppFeat: constr(regex=r"^[A-Fa-f0-9]*$")
