...
def read_active_subscriptions(
    *,
    ...
    """
    Read all active provisionings
    """ 
    db_mongo = client.fastapi
    retrieved_docs = crud_mongo.read_all(db_mongo, db_collection, current_user.id)

    #Check if there are any active provisionings
    if not retrieved_docs:
        raise HTTPException(status_code=404, detail="There are no active provisionings")
...
def read_subscription(
    *,
    ...
    try:
        retrieved_doc = crud_mongo.read_uuid(db_mongo, db_collection, provisioningId)
...
def update_subscription(
    *,
    ...
    """
    Update/Replace an existing provisioning resource by id
    """
    db_mongo = client.fastapi
    ...
    #Update the document
    json_data = jsonable_encoder(item_in)
    crud_mongo.update_new_field(db_mongo, db_collection, provisioningId, json_data)

    #Retrieve the updated document | UpdateResult is not a dict
    updated_doc = crud_mongo.read_uuid(db_mongo, db_collection, provisioningId)
...
def delete_subscription(
    *,
    ...
    crud_mongo.delete_by_uuid(db_mongo, db_collection, provisioningId)
    http_response = JSONResponse(content=retrieved_doc, status_code=200)
    add_notifications(http_request, http_response, False)
    return http_response